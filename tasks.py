...
# from test_attention import SAFM
from ultralytics.nn.modules.head import v10Detect
from ultralytics.nn.modules.block import SCDown, PSA
from ultralytics.nn.extra_modules.AttentionFusionModel import CrossAttentionFusion, SelfAttentionFusion, Add2
from ultralytics.nn.extra_modules.attention_fusion import *
...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect,v10Detect, Detect_TADDH, Detect_RSCD, Detect_SEAM,
                          Detect_Efficient, Segment)):
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    log_file_name = os.path.basename(d.get('yaml_file')).replace('.yaml', '.txt')
    log_base_path = r'/root/autodl-tmp/YOLO-MIF-master/runs/train/RGBT-ALL-dataset_IR'
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, Focus, SCDown, PSA, C2fCIB,
                 BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x, RepC3,
                 C2f_ACDBB,C2f_DeepACDBB,C2f_DeepDBB,C2f_DeepACDBBMix, C2f_WDBB,C2f_RDBB,C3_RDBB,C2f_MixedDBB,
                 C3_MixedDBB,C2f_ACNET,C3SECA,C2f_StripCGLU, PTSSA, RepHMS, C2f_GlobalFilter, C2f_DynamicFilter,
                 ELAN1, AConv, SPPELAN, C2fAttn, nn.Conv2d, C2f_Faster, C2f_ODConv,
                ...
                C2f_DCMB, C2f_DCMB_KAN, C2f_GlobalFilter, C2f_DynamicFilter, PTSSA, RepHMS
                 ):
            ...
            if m in (BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, C3x, RepC3, 
                     C2f_RDBB,C3_RDBB,C2f_MixedDBB,C3_MixedDBB, C2f_DBB, 
                     C2f_ACDBB,C2f_DeepACDBB,C2f_DeepDBB,C2f_DeepACDBBMix,C2f_ACNET,
                     C2fAttn, C2fCIB, C2f_Faster, C2f_ODConv, C2f_Faster_EMA, 
                     ...
                     C2f_DCMB, C2f_DCMB_KAN, C2f_GlobalFilter, C2f_DynamicFilter
                    ):
        ...
        elif m in (Detect, Detect_DyHead, Detect_AFPN_P2345, Detect_AFPN_P2345_Custom, Detect_AFPN_P345, Detect_AFPN_P345_Custom, Detect_Efficient, DetectAux, Detect_SEAM, Detect_MultiSEAM, 
                Detect_DyHeadWithDCNV3, Detect_DyHeadWithDCNV4, Detect_DyHead_Prune, Detect_LSCD, Detect_TADDH, Detect_LADH, Detect_LSCSBD, Detect_LSDECD, Detect_RSCD, Detect_SEAM,):
        ...
        # --------------ASF--------------
            ...
            c2 = ch[f[0]]
            args = [c2, args[1]]
        elif m in {ScalSeq, DynamicScalSeq, DynamicScalSeq_x2}:
            c1 = [ch[x] for x in f]
            c2 = make_divisible(args[0] * width, 8)
            args = [c1, c2]
        elif m in {asf_attention_model, asf_attention_model_x3, asf_attention_model_x4}:
            args = [[ch[x] for x in f]]
            c2 = ch[f[-1]]
        # --------------Fusion_attention--------------
        # elif m is AttentionFusion:
        #     c2 = sum(ch[x] for x in f)
        #     args = [ch[f[-1]]]
        # --------------ASF--------------
        elif m is SDI:
            ...
            c2 = int(c1[1] * 0.5 * 3)
            args = [c1, *args]
        elif m in {DASI, DASI_x2}:
            c1 = [ch[x] for x in f]
            c2 = c1[1]
            args = [c1, c2]
        elif m in {CSMHSA, CSMHSA_x2, CSMHSA_x3}:
            c1 = [ch[x] for x in f]
            c2 = ch[f[-1]]
            args = [c1, c2]
        elif m is CFC_CRB:
            ...
            c2 = c1 // 2
            args = [c1, *args]
        elif m in {SFC_G2, SFC_G2_Fusion, SFC_G2_x2, SFC_G2_x3, SFC_G2_x4, DualCrossAttention, TripleFusionAttention, QuadFusionAttention}:
            c1 = [ch[x] for x in f]
            # c2 = c1[0]
            c2 = c1[-1]
            args = [c1, *args]
        elif m in {CGAFusion, CGAFusion_T, CAFMFusion, CAFM_CGAFusion}:
            # c2 = ch[f[1]]
            # args = [c2, *args]
            c2 = ch[f[-1]]
            ...
            c2 = ch[f[1]]
            args = [c2, *args]
        elif m in {ContextGuideFusionModule, ContextGuideFusionModule_x3, ContextGuideFusionModule_x4}:
            c1 = [ch[x] for x in f]
            c2 = len(c1) * c1[-1]
            args = [c1]
        # elif m in {PSA}:
        ...
        # --------------Fusion--------------            
            ...
            c2 = c1[0]
            args = [c1]
        elif m in {MultiScaleGatedAttn, MultiScaleGatedAttn_x3, MultiScaleGatedAttn_x4}:
            c1 = [ch[x] for x in f]
            c2 = max(c1)
            args = [c1]
        elif m in {HAFB, HAFB_x3, HAFB_x4}:
            c1 = [ch[x] for x in f]
...
def guess_model_task(model):
    ...
    if isinstance(model, nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, (Detect, v10Detect, Detect_Efficient, Detect_TADDH,
                            Detect_RSCD, Detect_SEAM, )):
                return 'detect'
